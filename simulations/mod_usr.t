...
!  use mod_global_parameters
    ...
    double precision, dimension(jmax) :: x_i,y_i,z_i,s_i
    double precision, dimension(imax) :: x_o,y_o,z_o
    double precision, dimension(imax) :: dz,sumt
  
    Tcor=1.0d0   !> The temperature of solar corona
    Tpho=6.d-3   !> The temperature of photosphere
    htra=0.2719d0   !> The height of photosphere-corona transition region is 3.5Mm
    !wtra=0.025d0   !> The widget of the tanh function
    !wtra=0.04d0
    wtra=0.025d0 
    allocate(s_o(imax))
    allocate(g_o(imax))
...
!> Input the geometry configuration of MFR
 if(mype .eq. 0) then
      open(unit=55,file='path.dat',status='old')
      do j=1,jmax
        read(55, '(3e25.16)') x_i(j),y_i(j),z_i(j)
      enddo
      close(55)
  endif
...
!> Parallel computing procedure
    ...
    if(npe>1)then
      call MPI_BCAST(x_i,jmax,MPI_DOUBLE_PRECISION,0,icomm,ierrmpi)
      call MPI_BCAST(y_i,jmax,MPI_DOUBLE_PRECISION,0,icomm,ierrmpi)
      call MPI_BCAST(z_i,jmax,MPI_DOUBLE_PRECISION,0,icomm,ierrmpi)
    endif

!> 3D curve integration
    s_i=0
    do j=1,jmax
      if(j .gt. 1) then
    ...
    if(mype .eq. 0) then
      ...
      if(abs(loop-(xprobmax1-xprobmin1)) .gt. dxlv1) then
        call mpistop("please change xprobmax1 = loop")
      end if
    end if
    gzone=0.005d0*loop !> Setting the length of ghost zone 
    do i=1,imax
      s_o(i)=loop*1.01d0*(i-1)/(imax-1)-gzone
      !s_o(i)=loop*0.999*(i-1)/(imax-1)
    end do 
   ...
   do i=1,imax 
     do j=1,jmax-1
      ...
      x_o(i)=x_i(j)+(x_i(j+1)-x_i(j))*(sj-s_i(j))/(s_i(j+1)-s_i(j))  
      y_o(i)=y_i(j)+(y_i(j+1)-y_i(j))*(sj-s_i(j))/(s_i(j+1)-s_i(j))
      z_o(i)=z_i(j)+(z_i(j+1)-z_i(j))*(sj-s_i(j))/(s_i(j+1)-s_i(j))
      end if
     end do
   end do
...
!  use mod_global_parameters
    ...
    double precision, intent(inout) :: w(ixI^S,1:nw)
    double precision :: lQgrid(ixI^S),bQgrid(ixI^S),flQgrid(ixI^S)
    ...
    !call getflQ(flQgrid,ixI^L,ixO^L,qtC,wCT,x)
    !w(ixO^S,e_)=w(ixO^S,e_)+qdt*flQgrid(ixO^S)
...
!!!!!!!!!!lt!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!nosame
    if(qt .le. trelax) then
      lQgrid=zero
    else if(qt .le. trelax+trise) then
      lQgrid=(qt-trelax)/trise
    else
      !lQgrid=One
      !lQgrid=zero
      lQgrid=One
  ...
  subroutine getflQ(flQgrid,ixI^L,ixO^L,qt,w,x)
    use mod_global_parameters
    integer, intent(in) :: ixI^L, ixO^L
    double precision, intent(in) :: qt, x(ixI^S,1:ndim)
    double precision, intent(in) :: w(ixI^S,1:nw)
    double precision :: flQgrid(ixI^S),flQ0
    double precision :: t0,ts,sp,ss
    flQ0=8.0d-2/heatunit
    t0 = 500.0d0
    ts = 2.0d0
    flQgrid(ixO^S)= dexp(-((qt-t0)*(qt-t0)/(ts*ts)))
    sp = 1.0
    ss = 0.25
    flQgrid(ixO^S)=flQ0*dexp(-((x(ixO^S,1)-sp)*(x(ixO^S,1)-sp)/(ss*ss)))*flQgrid(ixO^S)
  end subroutine getflQ
